...
class BASEOBJECT(QObject):
    ...
    def safeloadprocessmodels(self):
        for item in static_data["transoptimi"]:
            name = item["name"]
            try:
                checkpath = "VTranslator/transoptimi/" + name + ".py"
    ...
    def GetTranslationCallback(
        ...
        with self.gettranslatelock:
            ...
            if (
                ...
                # Log the translation result with engine name and formatted coordinates
                try:
                ...
                # Clean coordinate tags for UI display
    ...
    def startoutputer(self):
        for classname in globalconfig["textoutputer"]:
            if not os.path.exists(
                "VTranslator/textio/textoutput/" + classname + ".py"
            ):
                continue
            ...
            try:
                ...
                if _fn.endswith(".py"):
                    style = importlib.import_module(
                        "files.VTranslator_qss." + _fn[:-3].replace("/", ".")
                    ).stylesheet()
                elif _fn.endswith(".qss"):
                    with open(
                        "files/VTranslator_qss/{}".format(_fn),
                        "r",
                    ) as ff:
                        style = ff.read()
            except:
    ...
    def urlprotocol(self):
        ...
        key = winreg.CreateKey(
            winreg.HKEY_CURRENT_USER, r"Software\Classes\vtranslator"
        )
        winreg.SetValue(key, None, winreg.REG_SZ, "URL:vtranslator")
        winreg.SetValueEx(key, r"URL Protocol", 0, winreg.REG_SZ, "")
        keysub = winreg.CreateKey(
            winreg.HKEY_CURRENT_USER,
            r"Software\Classes\vtranslator\shell\open\command",
        )
