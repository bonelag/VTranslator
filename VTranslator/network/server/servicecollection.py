...
class internalservicetranshistws(WSHandler, somecommon_2):
    ...
    def onmessage(self, message: str):
        ...
        dict(callvloadready=self.callvloadready)[function](*args)
...
class internalservicemainuiws(WSHandler, somecommon_1):
    ...
    def onmessage(self, message: str):
        ...
        args = message.get("args", tuple())
        dict(
            callvloadready=self.callvloadready,
            callvclickedword=gobject.base.clickwordcallback,
        )[function](*args)
...
class PageSearchWord(HTTPHandler):
    ...
    def parse(self, _: RequestInfo):
        page = r"VTranslator\htmlcode\service\dictionary.html"
...
class Pagetranslate(HTTPHandler):
    ...
    def parse(self, _: RequestInfo):
        page = r"VTranslator\htmlcode\service\translate.html"
        return FileResponse(page)
...
class Pageocr(HTTPHandler):
    ...
    def parse(self, _):
        page = r"VTranslator\htmlcode\service\ocr.html"
        return FileResponse(page)
...
class Pagetts(HTTPHandler):
    ...
    def parse(self, _):
        page = r"VTranslator\htmlcode\service\tts.html"
        return FileResponse(page)
...
class BasePage(HTTPHandler):
    ...
    def parse(self, _):
        return FileResponse(r"VTranslator\htmlcode\service\basepage.html")
