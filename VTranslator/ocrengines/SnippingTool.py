    ...
    def ocr(self, qimage: QImage):
        ...
        with self.lock:
            ...
            for i in range(cnt):
                ...
                # Ensure box coordinates are floats
                box_coords = (float(box.x1), float(box.y1), float(box.x2), float(box.y2), 
                              float(box.x3), float(box.y3), float(box.x4), float(box.y4))
                
                # Append each line as a separate block
                texts.append(text)
