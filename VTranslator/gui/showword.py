...
def cishusX():
    ...
    for K in globalconfig["cishu"]:
        if os.path.isfile("VTranslator/cishu/{}.py".format(K)):
            __.append(K)
    return __
...
class AnkiWindow(QWidget):
    ...
    def makedictionaryHTML(self, dictionarys):
        ...
        for iiii in range(len(dictionarys)):
            htmlcontents += (
                '<div id="v_dict_tab_{}" class="tab-pane">{}</div>'.format(
    ...
    def createaddtab(self):
        ...
        def refreshcombo(combo: QComboBox, changed):
            ...
            if len(globalconfig["ankiconnect"]["DeckNameS"]) == 0:
                globalconfig["ankiconnect"]["DeckNameS"].append("vdeck")
    ...
    def tryloadankitemplates(self):
        try:
            ...
            with open(
                r"VTranslator\htmlcode/anki/back.html", "r", encoding="utf8"
            ) as ff:
                model_htmlback = ff.read()
            with open(
                r"VTranslator\htmlcode/anki/front.html", "r", encoding="utf8"
            ) as ff:
                model_htmlfront = ff.read()
            with open(
                r"VTranslator\htmlcode/anki/style.css", "r", encoding="utf8"
            ) as ff:
                model_css = ff.read()
...
class WordViewer(QWidget):
    ...
    def __init__(self, parent=None, tabonehide=False, transp=False):
        ...
        self.textOutput.bind(
            "v_recheck_current_html", self.v_recheck_current_html
        )
        self.textOutput.bind("v_search_word", self.from_webview_search_word.emit)
        self.textOutput.bind(
            "v_audio_play_b64",
    ...
    def v_recheck_current_html(self, html):
    ...
    def tabclicked(self, idx):
        ...
        try:
            ...
            return
        with open(
            r"VTranslator\htmlcode\uiwebview\dictionary.html", "r", encoding="utf8"
        ) as ff:
            frame = ff.read()
        ...
        if self.use_bg_color_parser:
            ...
            backgroundparser = ""
        html = frame.replace("__v_dict_internal_view__", html).replace(
            "__v_dict_internal_handle_bgcolor__", backgroundparser
        )
