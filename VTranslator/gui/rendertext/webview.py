...
class TextBrowser(WebviewWidget, somecommon):
    ...
    def __init__(self, parent) -> None:
        ...
        self.bind("callvclickedword", gobject.base.clickwordcallback)
        self.bind("callvMouseMove", self.callvMouseMove)
        self.bind("callvMousePress", self.callvMousePress)
        self.bind("callvMouseRelease", self.callvMouseRelease)
        self.bind("callvSizeChanged", self.callvSizeChanged)
        self.bind("callvEnter", self.callvEnter)
        self.bind("callvLeave", self.callvLeave)
        self.bind("callvloadready", self.callvloadready)
        self.bind("callvMouseHoverWord", self.callvMouseHoverWord)
    ...
    def loadex_(extra=None):
        ...
        basepath = r"VTranslator\htmlcode\uiwebview\mainui.html"
        if not extra:
    ...
    def loadextra():
        ...
        for _ in [
            ...
            r"VTranslator\htmlcode\uiwebview\extrahtml\mainui.html",
        ]:
    ...
    def callvMousePress(self, btn, x, y):
    ...
    def callvMouseRelease(self, btn, x, y):
        pos = self.parsexyaspos(x, y)
        btn = self.parsemousebutton(btn)
        event = QMouseEvent(
    ...
    def callvMouseHoverWord(self, event, x, y, word: str):
