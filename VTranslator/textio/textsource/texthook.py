...
class texthook(basetext):
    ...
    def edit_selectedhook_insert(self, key, idx=-1):
        ...
        _, _, tp = key
        self.V_SyncThread(tp, True)

    def initdll(self):
        VHost = CDLL(
            gobject.GetDllpath(
                ("VHost32.dll", "VHost64.dll"),
                os.path.abspath("files/VHook"),
            )
        )
        self.V_SyncThread = VHost.V_SyncThread
        self.V_SyncThread.argtypes = ThreadParam, c_bool
        self.V_InsertPCHooks = VHost.V_InsertPCHooks
        self.V_InsertPCHooks.argtypes = (DWORD, c_int)
        self.V_Settings = VHost.V_Settings
        self.V_Settings.argtypes = c_int, c_bool, c_int, c_int, c_int
        self.V_Start = VHost.V_Start
        self.V_Start.argtypes = (
            ProcessEvent,
            ProcessEvent,
        ...
        self.V_AllocString = VHost.V_AllocString
        self.V_AllocString.argtypes = (c_wchar_p,)
        self.V_AllocString.restype = c_void_p
        self.V_ConnectProcess = VHost.V_ConnectProcess
        self.V_ConnectProcess.argtypes = (DWORD,)
        self.V_CheckIfNeedInject = VHost.V_CheckIfNeedInject
        self.V_CheckIfNeedInject.argtypes = (DWORD,)
        self.V_CheckIfNeedInject.restype = c_bool
        self.V_InsertHookCode = VHost.V_InsertHookCode
        self.V_InsertHookCode.argtypes = DWORD, LPCWSTR
        self.V_InsertHookCode.restype = c_bool
        self.V_RemoveHook = VHost.V_RemoveHook
        self.V_RemoveHook.argtypes = DWORD, c_uint64
        self.V_DetachProcess = VHost.V_DetachProcess
        self.V_DetachProcess.argtypes = (DWORD,)
        self.V_ResetLang = VHost.V_ResetLang
        self.V_FindHooks = VHost.V_FindHooks
        self.V_FindHooks.argtypes = (
            DWORD,
            SearchParam,
            FindHooksCallback_t,
            c_wchar_p,
        )
        self.V_EmbedSettings = VHost.V_EmbedSettings
        self.V_EmbedSettings.argtypes = (
            DWORD,
            c_uint32,
            c_uint8,
            c_bool,
        ...
        self.V_CheckIsUsingEmbed = VHost.V_CheckIsUsingEmbed
        self.V_CheckIsUsingEmbed.argtypes = (ThreadParam,)
        self.V_CheckIsUsingEmbed.restype = c_bool
        self.V_UseEmbed = VHost.V_UseEmbed
        self.V_UseEmbed.argtypes = ThreadParam, c_bool
        self.V_EmbedCallback = VHost.V_EmbedCallback
        self.V_EmbedCallback.argtypes = ThreadParam, LPCWSTR, LPCWSTR

        self.V_QueryThreadHistory = VHost.V_QueryThreadHistory
        self.V_QueryThreadHistory.argtypes = (ThreadParam, c_bool, c_void_p)
        procs = [
    ...
    def i18nQueryCallback(self, querytext: str):
        return self.V_AllocString(_TR(querytext))
    ...
    def listprocessm(self):
        ...
        for name_ in _list:
            ...
            if fn in ret:
                continue
            if fn.lower() in ["vhook32.dll", "vhook64.dll"]:
                continue
            ret.append(fn)
        return sorted(ret)
    ...
    def QueryThreadHistory(self, tp, _latest=False):
        ret = []
        self.V_QueryThreadHistory(tp, _latest, QueryHistoryCallback(ret.append))
        return ret[0]
    ...
    def start_unsafe(self, pids):
        ...
        injectpids = []
        for pid in pids:
            self.V_ConnectProcess(pid)
            if self.V_CheckIfNeedInject(pid):
                injectpids.append(pid)
        if len(injectpids):
            arch = ["32", "64"][self.is64bit]
            dll = os.path.abspath("files/VHook/VHook{}.dll".format(arch))
    ...
    def onprocconnect(self, pid):
        ...
        for hookcode in self.needinserthookcode:
            self.V_InsertHookCode(pid, hookcode)
        if self.hconfig.get("insertpchooks_string", False):
            self.V_InsertPCHooks(pid, 0)
            self.V_InsertPCHooks(pid, 1)
    ...
    def embedcallback(self, text: str, trans: str, tp: ThreadParam):
        ...
        self.V_EmbedCallback(tp, text, trans)
    ...
    def findhook(self, usestruct, addresses):
        ...
        for pid in pids:
            self.V_FindHooks(pid, usestruct, _callback, addresses)
    ...
    def inserthook(self, hookcode):
        ...
        for pid in self.pids.copy():
            succ = self.V_InsertHookCode(pid, hookcode) and succ
        if succ == False:
