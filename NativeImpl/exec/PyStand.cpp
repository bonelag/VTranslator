...
int PyStand::DetectScript()
{
	...
	std::wstring test;
	test = _home + L"\\VTranslator\\main.py";
	...
	SetEnvironmentVariableW(L"V_EXE_NAME", buffer.data());
	return 0;
}
...
int main()
{
	{
		// 当更新进行时，禁止启动
		CHandle hMutex{CreateMutex(NULL, FALSE, L"V_UPDATER_SINGLE")};
		if (GetLastError() == ERROR_ALREADY_EXISTS)
			return 0;
	}
	CHandle __handle{CreateMutexA(&allAccess, FALSE, "V_UPDATER_BLOCK")};
