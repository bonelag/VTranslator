...
static auto V_UPDATE_PREPARED_OK = RegisterWindowMessage(L"V_UPDATE_PREPARED_OK");
...
static LRESULT CALLBACK WNDPROC_1(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (message == V_UPDATE_PREPARED_OK)
    {
...
{
    const wchar_t CLASS_NAME[] = L"v_unique_classname_";
    static int idx = 0;
...
DECLARE_API void globalmessagelistener(WinEventHookCALLBACK_t callback1, WindowMessageCallback_t callback)
{
    ...
    ChangeWindowMessageFilterEx(hWnd, V_UPDATE_PREPARED_OK, MSGFLT_ALLOW, nullptr);
...
{
    PostMessage(HWND_BROADCAST, V_UPDATE_PREPARED_OK, 0, 0);
}
