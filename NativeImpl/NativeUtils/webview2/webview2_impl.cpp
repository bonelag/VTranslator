...
HRESULT STDMETHODCALLTYPE WebView2ComHandler::Invoke(HRESULT result, ICoreWebView2Controller *controller)
{
    [&]()
    {
        ...
        ref->m_webView->AddScriptToExecuteOnDocumentCreated(L"window.VJSObject={}", nullptr);
...
std::optional<std::wstring> WebView2::UserDir(bool loadextension)
{
    ...
    PathCombineW(userDataFolder, dataPath, loadextension ? L"VTranslator" : L"VTranslator_noext");
...
{
    std::wstring js = std::wstring{} + L"window.VJSObject." + funcname + L" = function(){window.chrome.webview.postMessage({    method: '" + funcname + L"',    args: Array.prototype.slice.call(arguments)});};";
