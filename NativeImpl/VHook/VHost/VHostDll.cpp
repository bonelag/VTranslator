#include "host.h"
#define C_V_API extern "C" __declspec(dllexport)
...
C_V_API void V_Start(ProcessEvent Connect, ProcessEvent Disconnect, ThreadEvent_maybe_embed Create, ThreadEvent Destroy, OutputCallback Output, HostInfoHandler hostinfo, HookInsertHandler hookinsert, EmbedCallback embed, I18NQueryCallback i18nQueryCallback)
{
    Host::Start(
...
#if 0
C_V_API void V_ConnectAndInjectProcess(DWORD pid)
{
    Host::ConnectAndInjectProcess(pid);
}
#endif
C_V_API bool V_CheckIfNeedInject(DWORD pid)
{
    return Host::CheckIfNeedInject(pid);
}
C_V_API void V_ConnectProcess(DWORD pid)
{
    Host::ConnectProcess(pid);
}
C_V_API void V_DetachProcess(DWORD pid)
{
    Host::DetachProcess(pid);
}
C_V_API void *V_AllocString(const wchar_t *str)
{
    if (!str)
        return nullptr;
...
C_V_API void V_Settings(int flushDelay, bool filterRepetition, int defaultCodepage, int maxBufferSize, int maxHistorySize)
{
...
C_V_API void V_ResetLang()
{
    Host::ResetLanguage();
}
C_V_API void V_InsertPCHooks(DWORD pid, int which)
{
    Host::InsertPCHooks(pid, which);
}
C_V_API bool V_InsertHookCode(DWORD pid, LPCWSTR hookcode)
{
...
C_V_API void V_QueryThreadHistory(ThreadParam tp, bool latest, void (*callback)(const wchar_t *))
{
    if (latest)
...
C_V_API void V_RemoveHook(DWORD pid, uint64_t addr)
{
    Host::RemoveHook(pid, addr);
}
C_V_API void V_FindHooks(DWORD pid, SearchParam sp, findhookcallback_t findhookcallback, LPCWSTR addresses)
{
...
C_V_API void V_EmbedSettings(DWORD pid, UINT32 waittime, UINT8 fontCharSet, bool fontCharSetEnabled, wchar_t *fontFamily, Displaymode displaymode, bool fastskipignore, bool clearText)
{
...
C_V_API bool V_CheckIsUsingEmbed(ThreadParam tp)
{
    return Host::CheckIsUsingEmbed(tp);
}
C_V_API void V_UseEmbed(ThreadParam tp, bool use)
{
...
C_V_API void V_EmbedCallback(ThreadParam tp, LPCWSTR text, LPCWSTR trans)
{
    auto sm = Host::GetCommonSharedMem(tp.processId);
    if (!sm)
        return;
    ...
    char eventname[1000];
    sprintf(eventname, V_EMBED_notify_event, tp.processId, simplehash::djb2_n2((const unsigned char *)(text), wcslen(text) * 2));
    win_event event1(eventname);
    event1.signal(true);
}

C_V_API void V_SyncThread(ThreadParam tp, bool sync)
{
