...
void VHost::on_proc_disconnect(DWORD pid)
{
    attachedprocess.erase(pid);
}

void VHost::on_proc_connect(DWORD pid)
{
...
VHost::VHost()
{
    ...
    // btnsavehook->onclick=std::bind(&VHost::btnsavehookscallback,this);
    ...
    Host::Start(
        std::bind(&VHost::on_proc_connect, this, std::placeholders::_1),
        std::bind(&VHost::on_proc_disconnect, this, std::placeholders::_1),
        std::bind(&VHost::on_thread_create, this, std::placeholders::_1),
        std::bind(&VHost::on_thread_delete, this, std::placeholders::_1),
        std::bind(&VHost::on_text_recv, this, std::placeholders::_1, std::placeholders::_2),
    ...
    setcentral(1200, 800);
    std::wstring title = TR[WndVHostGui];
    settext(title);
    ...
    wsprintf(vs, L" | %s v%d.%d.%d", TR[VersionCurrent], V_VERSION[0], V_VERSION[1], V_VERSION[2]);
    title += vs;
    settext(title);
    std::thread([&]()
                {
...
void VHost::on_text_recv_checkissaved(TextThread &thread)
{
...
void VHost::showtext(const std::wstring &text, bool clear)
{
...
void VHost::updatelisttext(const std::wstring &text, LONG_PTR data)
{
...
void VHost::on_text_recv(TextThread &thread, std::wstring &output)
{
    if (hasstoped)
        return;
...
void VHost::on_info(HOSTINFO type, const std::wstring &warning)
{
    switch (type)
    {
...
void VHost::on_thread_create(TextThread &thread)
{
    wchar_t buff[65535];
...
void VHost::on_thread_delete(TextThread &thread)
{
    if (currentselect == (LONG_PTR)&thread)
        currentselect = 0;
...
Settingwindow::Settingwindow(VHost *host) : mainwindow(host)
{
    int height = 30;
    int curry = 10;
    int space = 10;
