#include "VSoft.h"
/** jichi 12/27/2014 VSoft
 * Sample game: [141226] [VSoft] 悪堕ラビリンス -- /hsn8@46C5EF
 *
 * /hsn8@46C5EF
 * - addr: 0x46C5EF
 * - off: 8
...
// static bool VSoftFilter(LPVOID data, size_t *size, HookParam *)
//{
//  size_t len = *size;
...
bool InsertVSoftHook()
{
  ...
  if (!addr)
  {
    ConsoleOutput("VSoft: pattern not found");
    return false;
  }
  HookParam hp;
  ...
  // hp.filter_fun = VSoftFilter; // remove \n
  ConsoleOutput("INSERT VSoft");
  return NewHook(hp, "VSoft");

  // There are no GDI functions anyway
  // ConsoleOutput("VSoft: disable GDI hooks");
  //
}
bool InsertXXkata()
{
...
{ // unnamed
  namespace ScenarioHook
  {
    ...
    bool attach(ULONG startAddress, ULONG stopAddress) // attach scenario
    {
      ...
      auto succ = NewHook(hp, "EMBEDV");
      hp.address = addr2;
      succ |= NewHook(hp, "EMBEDV");
...
bool VSoft::attach_function()
{
  if (PrincessSoft())
    return true;
  bool b1 = InsertVSoftHook();
