...
const wchar_t *V_CONTENTBYPASS(const wchar_t *_);
#define MAGIC_SEND "\x01VFROMJS\x01"
#define MAGIC_RECV "\x01VFROMHOST\x01"
namespace
{
...
namespace
{
	bool hook_V_CONTENTBYPASS()
	{
		HookParam hp;
		hp.address = (uintptr_t)V_CONTENTBYPASS;
		hp.type = USING_STRING | NO_CONTEXT | CODEC_UTF16 | EMBED_ABLE;
		hp.text_fun = [](hook_context *context, HookParam *hp, auto *buffer, uintptr_t *split)
		{
...
#define fnRunv_maylocal "?Run@Script@v8@@QEAA?AV?$MaybeLocal@VValue@v8@@@2@V?$Local@VContext@v8@@@2@@Z"
	...
	void _interrupt_function(void *isolate, void *)
	{
		...
		std::string vjspatch = LoadResData(L"vjspatch", L"JSSOURCE");
		strReplace(vjspatch, "IS_PACKED", std::to_string(is_packed));
		strReplace(vjspatch, "IS_USECLIPBOARD", std::to_string(useclipboard));
		strReplace(vjspatch, "INTERNAL_HTTP_PORT", std::to_string(usehttp_port));
		NewFromUtf8(&v8string, isolate, vjspatch.c_str(), 1, -1);
