...
#include <dwrite.h>
#include <atlbase.h>
extern "C" __declspec(dllexport) const wchar_t *v_internal_renpy_call_host(const wchar_t *text, int split)
{
    return text;
}
bool V_checkisusingembed(uint64_t address, uint64_t ctx2, bool usingsplit)
{
...
extern "C" __declspec(dllexport) bool v_internal_renpy_call_is_embed_using(int split, bool usingsplit)
{
    return V_checkisusingembed((uint64_t)v_internal_renpy_call_host, split, usingsplit);
}
namespace
{
    struct PyObject;
    ...
    bool hook_internal_renpy_call_host()
    {
        ...
        hp_internal.address = (uintptr_t)v_internal_renpy_call_host;
        ...
        return NewHook(hp_internal, "v_internal_renpy_call_host") &&
...
extern "C" __declspec(dllexport) const wchar_t *v_internal_renpy_get_font()
{
