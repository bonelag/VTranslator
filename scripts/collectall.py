...
launch = f"../NativeImpl/builds/_{arch}"
launch += f"_{target}"
targetdir = rf"build\VTranslator_{arch}_{target}"
...
copycheck(os.path.join(launch, "VTranslator.exe"), targetdir)
copycheck(os.path.join(launch, "VTranslator_admin.exe"), targetdir)
with open(os.path.join(targetdir, "VTranslator_debug.bat"), "w") as ff:
    ff.write(
        r""".\VTranslator.exe
pause"""
    )
# copycheck(os.path.join(launch, "VTranslator_admin.exe"), targetdir)
# copycheck(os.path.join(launch, "VTranslator_debug.exe"), targetdir)
copycheck("./VTranslator", targetdir)
